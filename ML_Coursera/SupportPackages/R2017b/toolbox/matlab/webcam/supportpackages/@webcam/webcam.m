...
%   Copyright 2013-2016 The MathWorks, Inc.
        ...
        function obj = webcam(varargin)
            ...
            try
                ...
                if(isempty(varargin))% empty arguments
                    ...
                    inputarg = varargin{1};
                    if (matlab.webcam.internal.Utility.isCharOrScalarString(inputarg)) % If the input argument is a string/char - Device name is passed in.
                        devName = validateName(inputarg, deviceNames);
                        index = ismember(deviceNames, devName);
                        devID = uniqueIDs{index};
                    elseif (isnumeric(inputarg)) % If the input argument is numeric - device index is passed in.
    ...
    methods (Static, Hidden)
        ...
        % Wrapper function to start support package installer for Webcams
            ...
            matlab.addons.supportpackage.internal.explorer.showSupportPackages('USBWEBCAM', 'tripwire');
        end
...
function resolvedName = validateName(deviceName,deviceList)
    partials = deviceList(strncmpi(deviceName,deviceList,strlength(deviceName)));
    exacts = 1;
