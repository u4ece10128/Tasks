      ...
      '<tr>' +
        '<td class="search_result_desc">' +
          '<span class="search_title">' +
            '<a href="<%= resolveProductHelpDir(searchResult.product)%>/<%= searchResult.path %><%= appendSearchParameters(searchResult, index) %>"><%=searchResult.title %></a>' +
...
function getSearchResultsHtml(searchResults, searchTerm, highlightExpand) {
    var jsonData = {searchResults: searchResults};
    appendHighlightExpand(searchTerm, highlightExpand);
    _.extend(jsonData, {
